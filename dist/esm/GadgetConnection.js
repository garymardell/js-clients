            ...
            forwardSubscription: (request) => {
                return {
                    subscribe: (sink) => {
                        const input = { ...request, query: request.query || "" };
                        return {
                            unsubscribe: applyAsyncIterableIteratorToSink(applyLiveQueryJSONPatch(makeAsyncIterableIteratorFromSink((sink) => this.baseSubscriptionClient.subscribe(input, sink))), sink),
                        };
                    },
                };
            },
